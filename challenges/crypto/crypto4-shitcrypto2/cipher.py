...
def get_nums(s, n):
    ...
    print(sections[-1])
    ret = [to_num(x) for x in sections]
    print(ret)
    return ret
...
def encrypt(k, m, n):
    ...
    # generate value lists based of k and m
    m_vals, k_vals, c_vals = [], [], []
    for lst in m_val_list:
        m_vals += lst
    for lst in k_val_list:
        k_vals += lst
    # joins the lists into a single list of values
    c_vals = [
        encr_vals(m_vals[i], k_vals[i % len(k_vals)], n)
